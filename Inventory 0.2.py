...
def open_grn_form(title, inventory_list):
    ...
    def row(lbl, widget, r):
        tk.Label(form, text=lbl).grid(row=r, column=0, sticky="w", pady=6)
    ...
    tk.Label(form, text="Rs / KG").grid(row=6, column=2)
    tk.Label(form, textvariable=transport_kg).grid(row=6, column=3)

    row("Other costs (Total Rs)",
        tk.Entry(form, textvariable=other_var,
                 validate="key", validatecommand=(vcmd, "%P")), 7)
    tk.Label(form, text="Rs / KG").grid(row=7, column=2)
    tk.Label(form, textvariable=other_kg).grid(row=7, column=3)

    tk.Label(form, text="Net rate per KG",
             font=("Arial", 10, "bold")).grid(row=8, column=0, pady=6)
    tk.Label(form, textvariable=net_rate,
             font=("Arial", 10, "bold")).grid(row=8, column=1)

    tk.Label(form, text="Total Cost (Rs)",
             font=("Arial", 10, "bold")).grid(row=9, column=0)
    tk.Label(form, textvariable=total_cost,
             font=("Arial", 10, "bold")).grid(row=9, column=1)
    ...
    tk.Button(btns, text="Submit GRN", width=15).pack(side="left", padx=10)
    tk.Button(btns, text="Clear", width=15,
              command=clear_fields).pack(side="left", padx=10)
    tk.Button(btns, text="Cancel", width=15,
...
def open_grn_window():
    ...
    tk.Button(win, text="Raw material (GRN)", height=2,
              command=lambda: open_grn_form(
                  "Raw material Good receive Note",
                  ["p1","p2","p3","p4","p5","p6","p7"])
              ).pack(fill="x", padx=40, pady=5)

    tk.Button(win, text="Waste (PNWi)", height=2,
              command=lambda: open_grn_form(
                  "Waste Good receive Note",
                  ["W1","W2","W3"])
              ).pack(fill="x", padx=40, pady=5)

    tk.Button(win, text="Finish Good (PNi)", height=2,
              command=lambda: open_grn_form(
                  "Finish Good receive Note",
                  ["Fg1","Fg2","Fg3","Fg4","Fg5","Fg6","Fg7"])
              ).pack(fill="x", padx=40, pady=5)

    tk.Button(win, text="Packing Materials (GRNpm)", height=2,
              command=lambda: open_grn_form(
...
def open_min_window():
    ...
    for b in ["Raw material (PNo)", "Waste (WSN)",
        ...
        tk.Button(win, text=b, height=2).pack(fill="x", padx=40, pady=5)
...
def main_window():
    ...
    rc = card(r,"Reports")
    for t in ["Bin Card","Goods Received Notes","Material Issue Note"]:
        tk.Button(rc,text=t,**style).pack(fill="x",padx=30,pady=10)
